...
class Invaders(GameApp):
    ...
    #
    # Attribute _lifeCounter: the number of lives players wasted
    # Invariant: _lifeCounter is an int
    #
    ...
    """
    Attribute lastKeys: the number of keys pressed last time frame
    Invariant: lastKeys is an int and lastKeys >= 0
    """
    ...
    def start(self):
        ...
        self.lastKeys = 0
        self._lifeCounter = 0
        self._text = GLabel(text = "Press \'S\' to Play", font_size = 70,
                            x = GAME_WIDTH // 2, y = GAME_HEIGHT //2,
                            linecolor=[1,1,1,1], fillcolor=[0.2,0.6,0.7,0.4])
    ...
    def update(self,dt):
        ...
        """
        # IMPLEMENT ME
        # Determine the Key Press
        if self._state == STATE_INACTIVE:
            self._determineKeyPress()
    ...
    def _determineKeyPress(self):
        """
        Determine the current state of the invaders and assign the state to
        the attribute self._state

        This method checks for a key press, and that changes the state to the
        next value. A key press is when a key is pressed for the first time.
        The user must release the key and press it again to change the state,
        since holding on the press would not be counted as new presses.
        """
        # Determine the current number of keys pressed
        currentKeys = self.input.key_count
        # Only change if just pressed the keys this animation frame
        change = currentKeys > 0 and self.lastKeys == 0

        if change and self._state == STATE_INACTIVE:
            # Click happened. Change the state
            self._state = STATE_NEWWAVE

        # Update attribute lastKeys
        self.lastKeys = currentKeys


    def _stateNewWave(self):
        """
        Creates a new wave and shows it on the screen.

        This method makes the STATE_NEWWAVE only last for one frame and swtich
        into STATE_ACTIVE in the second frame.
        """
        self._text = None
        self._wave = Wave()
    ...
    def _stateActive(self, dt):
        """
        Runs the animation as the state is active.

        This method determins the states of the game be checking whether the
        game is end (STATE_COMPLETE) as either the player shot all the aliens or
        the aliens marched over the dline. It also checks whether it need to
        pause the game when the ship explode but the player still have extra
        lives.
        ...
        if self._wave.winner == True:
            self._text = GLabel(text = "Congrats! You Win!", font_size = 60,
                                x = GAME_WIDTH // 2, y = GAME_HEIGHT //2,
                                linecolor=[1,1,1,1], fillcolor=[0.2,0.6,0.7,0.4])
            self._state = STATE_COMPLETE
        else:
            if self._wave.touchline == True:
                self._text = GLabel(text = "GAME OVER. You Lose", font_size = 60,
                                    x = GAME_WIDTH // 2, y = GAME_HEIGHT //2,
                                    linecolor=[1,1,1,1],
                                    fillcolor=[0.2,0.6,0.7,0.4])
                self._state = STATE_COMPLETE
            elif self._lifeCounter <= 2:
                if self._wave.animator == None and self._wave.lifeReducer == True:
                    self._state = STATE_PAUSED
                    self.lastKeys = self.input.key_count
                    self._lifeCounter += 1
                    if self._lifeCounter<=2:
                        self._text = GLabel(text = "Press \'s\' to continue",
                                        font_size = 60, x = GAME_WIDTH // 2,
                                        y = GAME_HEIGHT //2,linecolor=[1,1,1,1],
                                        fillcolor=[0.2,0.6,0.7,0.4])
                    else:
                        self._text = GLabel(text = "GAME OVER. You Lose",
                        font_size = 60,x = GAME_WIDTH // 2, y = GAME_HEIGHT //2,
                        linecolor=[1,1,1,1], fillcolor=[0.2,0.6,0.7,0.4])
                        self._state = STATE_COMPLETE
    ...
    def _statePaused(self, dt):
        """
        Runs the animation as the state is paused.
        ...
        # Determine the current number of keys pressed
        # Only change if just pressed the keys this animation frame
        if self.input.key_count > self.lastKeys:
            # Click happened. Change the state
            self._state = STATE_ACTIVE
            self._wave._ship = Ship()
            self._wave._playerBolt = []
            self._wave.lifeReducer = False
            self._wave.update(self.input, dt)
